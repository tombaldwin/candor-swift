...
Writes `<dir>/.candor/report.<pkg>.Swift.json` (the spec-0.35 envelope) plus two sidecars:
...
project is on candor-swift 0.32.0 (spec 0.35)". (On an older build that predates `--version`, read
...
**Report shape:** `{ "candor": {…, "spec": "0.35"}, "package": "<name>", "functions": [...] }` — an
