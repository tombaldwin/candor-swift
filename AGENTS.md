...
Writes `<dir>/.candor/report.<pkg>.Swift.json` (the spec-0.26 envelope) plus two sidecars:
...
project is on candor-swift 0.23.1 (spec 0.26)". (On an older build that predates `--version`, read
...
**Report shape:** `{ "candor": {…, "spec": "0.26"}, "package": "<name>", "functions": [...] }` — an
