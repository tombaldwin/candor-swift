...
final class CallCollector: SyntaxVisitor {
    ...
    //
    // THAT ARGUMENT IS RIGHT ABOUT THE SECOND MAP AND WAS A FABRICATION IN THE FIRST. A `let p` inside a
    ...
    //     }
    //
    // `movedNames` does not cover it, because a shadow is not a REBIND — nothing assigns to the outer `p`,
    // so there is nothing for the move pre-pass to record. `execLocatorInvisible` does not cover it either:
    // the outer handle takes no write at all here, so no refusal is ever raised. The gate is the binder
    // COUNT (`multiplyBoundNames`) — a name with two binder sites in one unit is not a name a body-wide
    // claim can be made about — and it lives at the READ (`recordProcessRun`) rather than at the write, so
    // the suppressing half stays monotone.
