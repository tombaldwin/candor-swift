...
`Type.property` and carrying `unitKind: "accessor"` (spec 0.12, informative); ordinary
...
Writes `<dir>/.candor/report.<pkg>.Swift.json` (the spec-0.12 envelope) plus two sidecars:
...
project is on candor-swift 0.12.0 (spec 0.12)". (On an older build that predates `--version`, read
...
**Report shape:** `{ "candor": {…, "spec": "0.12"}, "package": "<name>", "functions": [...] }` — an
